...
class SingleAxisTrajectory:
    ...
    def get_min_max_acc(self, t1, t2):
        ...
        for i in [0, 1]:
            if self._accPeakTimes[i] <= t1:
                continue
            if self._accPeakTimes[i] >= t2:
                continue
...
class RapidTrajectory:
    ...
    def _check_input_feasibility_section(
        ...
        if (fmin < fminAllowed) or (fmax > fmaxAllowed) or (wBound > wmaxAllowed):
            ...
            r1 = self._check_input_feasibility_section(
                fminAllowed, fmaxAllowed, wmaxAllowed, minTimeSection, t1, tHalf
            )

            if r1 == InputFeasibilityResult.Feasible:
                # check the other half
                return self._check_input_feasibility_section(
                    fminAllowed, fmaxAllowed, wmaxAllowed, minTimeSection, tHalf, t2
                )
            else:
    ...
    def check_position_feasibility(self, boundaryPoint, boundaryNormal):
        ...
        for i in range(3):
            coeffs[0] += (
                boundaryNormal[i] * self._axis[i].get_param_alpha() / 24.0
            )  # t**4
            coeffs[1] += (
                boundaryNormal[i] * self._axis[i].get_param_beta() / 6.0
            )  # t**3
            coeffs[2] += (
                boundaryNormal[i] * self._axis[i].get_param_gamma() / 2.0
            )  # t**2
            coeffs[3] += (
                boundaryNormal[i] * self._axis[i].get_initial_acceleration()
            )  # t
    ...
    def get_body_rates(self, t, dt=1e-3):
        ...
        **To get (p,q,r) rates, rotate these with the vehicle's attitude.**
        ...
        if np.linalg.norm(crossProd) > 1e-6:
            return (
                np.arccos(np.dot(n0, n1)) / dt * (crossProd / np.linalg.norm(crossProd))
            )
        else:
    ...
    def get_cost(self):
        ...
        """
        return (
            self._axis[0].get_cost()
            + self._axis[1].get_cost()
