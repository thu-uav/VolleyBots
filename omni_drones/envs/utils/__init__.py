...
def create_obstacle(
    ...
    kit_utils.set_collision_properties(prim_path, contact_offset=0.02, rest_offset=0)
...
def create_bar(
    ...
    joint.GetAttribute("physics:upperLimit").Set(1.0)
    joint.GetAttribute("physics:lowerLimit").Set(-1.0)
    ...
    def setup_joint(joint, attributes):
        drives = set(
            [key.split(":")[1] for key in attributes.keys() if key.startswith("drive")]
        )
        for drive in drives:
