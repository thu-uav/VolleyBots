...
import numpy as np
import torch
from scipy.interpolate import splev as _splev_scipy_impl
from scipy.interpolate import splint as _splint_scipy_impl
...
def splev_torch(
    x: torch.Tensor, t: torch.Tensor, c: torch.Tensor, k: int, der: int = 0
):
    """
    Evaluate a B-spline or its derivatives.
    ...
    if der == 0:
        ...
        assert (
            der <= k
        ), "The order of derivative to compute must be less than or equal to k."
        n = c.size(-2)
        return k * splev_torch(
            x,
            t[..., 1:-1],
            (c[..., 1:, :] - c[..., :-1, :])
            / (t[k + 1 : k + n] - t[1:n]).unsqueeze(-1),
            k - 1,
            der - 1,
        )
...
def _splev_torch_impl(x: torch.Tensor, t: torch.Tensor, c: torch.Tensor, k: int):
    ...
    """
    assert (
        t.size(0) == c.size(0) + k + 1
    ), f"{len(t)} != {len(c)} + {k} + {1}"  # m= n + k + 1

    x = torch.atleast_1d(x)
    assert (
        x.dim() == 1 and t.dim() == 1 and c.dim() == 2
    ), f"{x.shape}, {t.shape}, {c.shape}"
    n = c.size(0)
...
def init_traj(
    ...
    ctps_inter = ctps_2.unsqueeze(-2) + (ctps_n_2 - ctps_2).unsqueeze(
        -2
