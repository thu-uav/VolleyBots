...
class MixedEncoder(nn.Module):
    ...
    def forward(self, x: TensorDict):
        feats = [
            self.vision_encoder(x[obs_name])
            for obs_name in x.keys()
            if obs_name in self.vision_obs_names
        ]
        if self.state_encoder is not None:
            # TODO: attn_encoder with TensorDict input
            feats += [
                self.state_encoder(x[obs_name])
                for obs_name in x.keys()
...
class MobileNetV3Small(nn.Module):
    ...
    def _setup_backbone(self):
        ...
        self._backbone = nn.Sequential(full_backbone.features, full_backbone.avgpool)
