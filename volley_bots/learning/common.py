...
class MyBuffer:
    ...
    def __init__(self, max_size: int = 1000, device: torch.device = None):
    ...
    def sample(self, batch_size, seq_len: int = 80):
        ...
        sub_sample_idx = torch.randint(
            0, self.storage._storage.shape[1], (num_samples,)
        )
...
from .modules.networks import ENCODERS_MAP, MLP, VISION_ENCODER_MAP, MixedEncoder
...
def make_encoder(cfg, input_spec: TensorSpec) -> nn.Module:
    if isinstance(input_spec, (BoundedTensorSpec, UnboundedContinuousTensorSpec)):
    ...
    ):  # FIXME: add logic for composite spec with visual input and other inputs
        state_spec_dict = {}
        vision_spec_dict = {}
        ...
        if len(vision_spec_dict) == 0:
            ...
            raise NotImplementedError(
                "Multiple visual inputs are not supported for now (cuz this author is lazy)"
            )
    else:
