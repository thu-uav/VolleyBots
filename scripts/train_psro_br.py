...
def train(
    ...
    for file_path in file_paths:
        ...
        if file_path == file_paths[-1]:
            for i in range(len(policy.population_0)):
                policy.population_1.add_actor(actor_dict_0)

    assert len(policy.population_0) == len(policy.population_1)
    max_population_size = len(policy.population_0) + 1
    policy.current_player_id = 1
    ...
    if cfg.get("load_meta_policy_path") is not None:
        meta_policy_dict = np.load(cfg.get("load_meta_policy_path"))
        meta_policy_0 = meta_policy_dict["meta_policy_0"]
        assert meta_policy_0.shape[0] == len(policy.population_0)
        meta_policy_1 = meta_policy_dict["meta_policy_1"]
        assert meta_policy_1.shape[0] == len(policy.population_1)
        logging.info(f"Load meta-policy from {cfg.get('load_meta_policy_path')}")
    else:
        ...
        logging.info(f"Initialize meta-policy with uniform distribution.")
...
@hydra.main(version_base=None, config_path=CONFIG_PATH, config_name="train_psro_br")
